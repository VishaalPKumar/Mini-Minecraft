...
void ShaderProgram::drawInterleaved(Drawable &d, BufferType vboBufferType, BufferType idxBufferType) {
    if(d.elemCount(idxBufferType) < 0) {
        std::cout << "Attempting to draw a Drawable that has not initialized its count variable! Remember to set it to the length of your index array in create()."                     "Attempting to draw a Drawable that has not initialized its count variable! Remember to set it to the length of your index array in create()." << std::endl;
        return;
    }
    useMe();

    if (d.bindBuffer(vboBufferType)) {
        GLint attrPos = m_attribs["vs_Pos"];
        GLint attrNor = m_attribs["vs_Nor"];
        GLint attrUV = m_attribs["vs_UV"];
        GLint attrAnimate = m_attribs["vs_Animate"];


        int distBetween = sizeof(VertexData);
        if (attrPos != -1) {
            context->glEnableVertexAttribArray(attrPos);
            context->glVertexAttribPointer(attrPos, 4, GL_FLOAT, false, distBetween, (void*)0);
        }

        if (attrNor != -1) {
            context->glEnableVertexAttribArray(attrNor);
            context->glVertexAttribPointer(attrNor, 4, GL_FLOAT, false, distBetween, (void*)(sizeof(glm::vec4)));
        }

        if (attrUV != -1) {
            context->glEnableVertexAttribArray(attrUV);
            context->glVertexAttribPointer(attrUV, 2, GL_FLOAT, false, distBetween, (void*)(2 * sizeof(glm::vec4)));
        }

        if (attrAnimate != -1) {
            context->glEnableVertexAttribArray(attrAnimate);
            context->glVertexAttribPointer(attrAnimate, 1, GL_FLOAT, false, distBetween, (void*)(2 * sizeof(glm::vec4) + sizeof(glm::vec2)));
        }
    }

    d.bindBuffer(idxBufferType);
    context->glDrawElements(d.drawMode(), d.elemCount(idxBufferType), GL_UNSIGNED_INT, 0);

    if (m_attribs["vs_Pos"] != -1) context->glDisableVertexAttribArray(m_attribs["vs_Pos"]);
    if (m_attribs["vs_Nor"] != -1) context->glDisableVertexAttribArray(m_attribs["vs_Nor"]);
    if (m_attribs["vs_UV"] != -1) context->glDisableVertexAttribArray(m_attribs["vs_UV"]);
    if (m_attribs["vs_Animate"] != -1) context->glDisableVertexAttribArray(m_attribs["vs_Animate"]);
    if (m_attribs["vs_PlayerPos"] != -1) context->glDisableVertexAttribArray(m_attribs["vs_PlayerPos"]);

    // context->printGLErrorLog();
}

void ShaderProgram::drawOpaque(Drawable &d) {
    drawInterleaved(d, INTERLEAVED, INDEX);
}

void ShaderProgram::drawTransparent(Drawable &d) {
    drawInterleaved(d, INTERLEAVED_TRANS, INDEX_TRANS);
}
