...
public:
    ...
    static int LOAD_DISTANCE;
    static int ZONE_SIZE;

    float maxHeight(int x, int z);
    ...
    void generateTerrainZone(int minX, int minZ);
    ...
    void spawnBlockTypeThread(int x, int z, Chunk* c);
    void spawnVBOThread(Chunk* c);

    // Initializes the Chunks that store the 64 x 256 x 64 block scene you
    // see when the base code is run.
    void CreateInitialScene();
    void updateLoadedChunks(int playerX, int playerZ);
    friend class Player; // TODO: Remove this!
};
